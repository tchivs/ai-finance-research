...
def frontmatter(title: str, description: str, weight: int, source_file: str, kind: str) -> str:
    return (
        "---\n"
        f"title: {quote(title)}\n"
...
def write_page(path: Path, title: str, description: str, weight: int, source: Path, body: str, kind: str) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        frontmatter(title, description, weight, str(source.relative_to(ROOT)), kind)
...
def main() -> None:
    ...
    for weight, project in enumerate(projects, start=1):
        ...
        write_page(project_dir / "_index.md", title, description, weight, quick, index_body, "project")
        write_page(
